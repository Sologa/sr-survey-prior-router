# PRISMA 2020 statement (subpage_checklist)

Source URL: https://www.prisma-statement.org/prisma-2020-checklist
Local raw file: `docs/agent_capability_packs/sr-survey-prior-router/references/canonical_sources/raw/prisma_2020/subpage_checklist_prisma-2020-checklist.html`

PRISMA 2020 checklist — PRISMA statement

0

Skip to Content

PRISMA 2020

PRISMA extensions

PRISMA-Check

Translations

Endorsement

Open Menu Close Menu

PRISMA 2020

PRISMA extensions

PRISMA-Check

Translations

Endorsement

Open Menu Close Menu

PRISMA 2020

PRISMA extensions

PRISMA-Check

Translations

Endorsement

# PRISMA 2020 checklist

The PRISMA 2020 statement comprises a checklist of items and sub-items, and an expanded checklist that details reporting recommendations for each item/sub-item.

-
PRISMA 2020 checklist ( PDF , Word )

-
PRISMA 2020 expanded checklist ( PDF )

The checklist can also be completed using a Shiny App available at [https://prisma.shinyapps.io/checklist/](https://prisma.shinyapps.io/checklist/) or online via the EQUATOR Network’s GoodReports website ( [https://www.goodreports.org/reporting-checklists/prisma/](https://www.goodreports.org/reporting-checklists/prisma/) ).

Note: The PRISMA 2020 checklists are distributed in accordance with the terms of the Creative Commons Attribution (CC BY 4.0) license, which permits others to distribute, remix, adapt and build upon this work, for commercial use, provided the original work is properly cited. To view a copy of this license, visit [https://creativecommons.org/licenses/by/4.0/](https://creativecommons.org/licenses/by/4.0/)

Copyright © 2024-2026 the PRISMA Executive . Contact: prismastatement@gmail.com
